...
Do CMPA přibyla volba Použít hlavní CS pro cílení internetu. Při jejím použití, pageviews v načteném Excelu reprezentují pageviews na hlavní <abbr title="Cílová skupina">CS</abbr>. Funguje jako behaviorální cílení internetu.
...
Vybraná <abbr title="Uživatelská skupina médií">USM</abbr> se dá dvojklikem (příp. přes pravé tlačítko a vybrání volby Upravit nebo stiskem klávasy F4) otevřít a upravovat přímo ze Zadání a v <abbr title="Editor definic částí dnů">EDČD</abbr>.
